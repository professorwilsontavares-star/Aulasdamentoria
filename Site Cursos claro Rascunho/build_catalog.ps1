...
$head_end = $html.IndexOf("</head>") + 7
$head = $html.Substring(0, $head_end)

$header_start = $html.IndexOf("<header>")
$header_end = $html.IndexOf("</header>") + 9
$header = $html.Substring($header_start, $header_end - $header_start)

$footer_start = $html.IndexOf("<footer")
$footer_end = $html.IndexOf("</footer>") + 9
$footer = $html.Substring($footer_start, $footer_end - $footer_start)

$catalog_start = $html.IndexOf("<section id=`"cursos`"")
$catalog_header_end = $html.IndexOf("<div class=`"tabs-list`"")
$catalog_header = $html.Substring($catalog_start, $catalog_header_end - $catalog_start)
...
<script>
    ...
    categoriasCatalogo.forEach((cat, index) => {
        ...
        if (!isSelected) tab.setAttribute("tabindex", "-1");
        ...
        if (!isSelected) panel.setAttribute("hidden", "true");
    ...
    tabTriggers.forEach(trigger => {
        ...
        trigger.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowRight') newIndex = (index + 1) % tabTriggers.length;
            else if (e.key === 'ArrowLeft') newIndex = (index - 1 + tabTriggers.length) % tabTriggers.length;
...
$script_part
"
Set-Content -Path "catalogo.html" -Value $new_html -Encoding UTF8
